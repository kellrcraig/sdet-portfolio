Feature: Products
    ...
    @page-navigation
    Scenario: Back to products link navigates from the Item detail screen to Products page
    @reset-app-state
    Scenario: Reset App State link reverts items, sorting option, and cart icon count to defaults
    Scenario: Items and sorting option persist when returning to Products page
    @cart-button-state
    Scenario: Remove button displays Add to cart after removing item from the cart
    @cart-button-state
    Scenario: Add to cart button displays Remove on Products page after adding from item detail
    @cart-button-state
    Scenario: Add to cart button displays Add to cart on Products page after removing from item detail
